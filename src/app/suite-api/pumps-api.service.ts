...
export class PumpsApiService {
  ...
  getPsatModuleFromInputs(psatInput: PsatInputs) {
    let pumpStyle = this.suiteApiEnumHelperService.getPumpStyleEnum(psatInput.pump_style);
    let pumpEfficiency = psatInput.pump_specified / 100;
    let rpm = psatInput.motor_rated_speed;
    let drive = this.suiteApiEnumHelperService.getDriveEnum(psatInput.drive);
    ...
    let speed = this.suiteApiEnumHelperService.getFixedSpeedEnum(psatInput.fixed_speed);
    ...
    //motor
    let lineFrequency = this.suiteApiEnumHelperService.getLineFrequencyEnum(psatInput.line_frequency);
    let motorRatedPower = psatInput.motor_rated_power;
    let motorRpm = psatInput.motor_rated_speed;
    let efficiencyClass = this.suiteApiEnumHelperService.getMotorEfficiencyEnum(psatInput.efficiency_class);
    ...
    let loadEstimationMethod = this.suiteApiEnumHelperService.getLoadEstimationMethod(psatInput.load_estimation_method);
  ...
  achievableEfficiency(pumpStyle: number, specificSpeed: number): number {
    let pumpStyleEnum = this.suiteApiEnumHelperService.getPumpStyleEnum(pumpStyle);
  ...
  estimateFla(motorRatedPower: number, motorRPM: number, frequency: number, efficiencyClass: number, efficiency: number, motorVoltage: number): number {
    let lineFrequency = this.suiteApiEnumHelperService.getLineFrequencyEnum(frequency);
    let motorEfficiencyEnum = this.suiteApiEnumHelperService.getMotorEfficiencyEnum(efficiencyClass);
  ...
  motorPerformance(lineFreq: number, efficiencyClass: number, motorRatedPower: number, motorRPM: number, specifiedEfficiency: number, motorRatedVoltage: number, fullLoadAmps: number, loadFactor: number): MotorPerformanceResults {
    let lineFrequency = this.suiteApiEnumHelperService.getLineFrequencyEnum(lineFreq);
    let motorEfficiencyClass = this.suiteApiEnumHelperService.getMotorEfficiencyEnum(efficiencyClass);
  ...
  nema(lineFreq: number, motorRPM: number, efficiencyClass: number, efficiency: number, motorRatedPower: number): number {
    let lineFrequency = this.suiteApiEnumHelperService.getLineFrequencyEnum(lineFreq);
    let efficiencyClassEnum = this.suiteApiEnumHelperService.getMotorEfficiencyEnum(efficiencyClass);
